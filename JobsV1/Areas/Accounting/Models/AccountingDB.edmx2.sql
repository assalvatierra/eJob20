...
-- --------------------------------------------------
-- Date Created: 03/22/2019 15:52:00
...
GO
IF OBJECT_ID(N'[dbo].[AccntMain12]', 'U') IS NOT NULL
    DROP TABLE [dbo].[AccntMain12];
GO
...
-- Creating primary key on [Id] in table 'AccntMain12'
ALTER TABLE [dbo].[AccntMain12]
ADD CONSTRAINT [PK_AccntMain12]
    PRIMARY KEY CLUSTERED ([Id] ASC);
GO
...
ALTER TABLE [dbo].[AccntMains]
ADD CONSTRAINT [FK_AccntTypeAccntMain]
    FOREIGN KEY ([AccntTypeId])
    REFERENCES [dbo].[AccntTypes]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_AccntTypeAccntMain'
CREATE INDEX [IX_FK_AccntTypeAccntMain]
ON [dbo].[AccntMains]
    ([AccntTypeId]);
GO
...
-- Creating foreign key on [AccntMainId1] in table 'AccntLedgers'
ALTER TABLE [dbo].[AccntLedgers]
ADD CONSTRAINT [FK_AccntMainAccntLedger]
    FOREIGN KEY ([AccntMainId1])
