        ...
        public List<cUserPerformance> GetUserPerformanceReport(DateTime sdate, DateTime edate)
        {
           ...
           edate = tempDate.AddDays(1);
            ...
            string sql =
               ...
               "         Amount = (SELECT ISNULL(SUM(Amount),0) FROM CustEntActivities ca WHERE ca.ActivityType = 'Sales' ca.Status = 'Awarded' AND AND au.UserName = ca.Assigned AND convert(datetime, ca.Date) > convert(datetime,'" + sdate + "') AND convert(datetime, ca.Date) < convert(datetime,'" + edate + "') )," +
